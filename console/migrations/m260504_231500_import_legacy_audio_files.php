...
final class m260504_231500_import_legacy_audio_files extends Migration
{
    public function safeUp(): void
    {
        ...
        try {
            foreach ($items as $item) {
                $sourceAbsolutePath = $this->createSourceAbsolutePath($item['legacy_path']);
                ...
                $this->replaceRecordingAudioMedia(
                    $item['recording_id'],
                    $mediaAssetId,
                );
                $this->clearLegacyDescription($item['recording_id'], $item['legacy_path'], $timestamp);
            }
    ...
    private function findImportItems(): array
    {
        ...
        $recordingQuery = (new Query())
            ...
            ->from('{{%recording}}')
            ->andWhere(['recording_type' => 'audio'])
            ->andWhere(['like', 'description', 'Legacy audio path:']);

        foreach ($recordingQuery->each() as $row) {
            $legacyPath = $this->extractLegacyPath((string) $row['description']);
        ...
        foreach ($recordingRows as $recordingRow) {
            ...
            $items[] = [
                ...
                'song_slug' => $songSlug,
                'legacy_path' => $recordingRow['legacy_path'],
            ];
        }
    ...
    private function extractLegacyPath(string $description): string | null
    {
        $prefix = 'Legacy audio path:';

        if (str_starts_with($description, $prefix) === false) {
            return null;
        }

        $legacyPath = trim(substr($description, strlen($prefix)));
    ...
    private function createTargetPath(
        ...
        return 'recordings/audio/' . $songSlug . '/' . $fileName;
    }
    ...
    private function copySourceFile(
        ...
        if (is_file($sourceAbsolutePath) === false) {
            throw new RuntimeException('Legacy audio file was not found: ' . $sourceAbsolutePath);
        }
        ...
        if (is_file($targetAbsolutePath) === true) {
            ...
            throw new RuntimeException('Target audio file already exists with different content: ' . $targetAbsolutePath);
        }

        if (copy($sourceAbsolutePath, $targetAbsolutePath) === false) {
            throw new RuntimeException('Failed to copy legacy audio file into uploads: ' . $targetAbsolutePath);
        }
    ...
    private function replaceRecordingAudioMedia(int $recordingId, int $mediaAssetId): void
    {
    ...
    private function clearLegacyDescription(int $recordingId, string $legacyPath, int $timestamp): void
    {
        $expectedDescription = 'Legacy audio path: ' . $legacyPath;
    ...
    {
        return Yii::getAlias('@storage/uploads/legacy-audio-source');
    }
